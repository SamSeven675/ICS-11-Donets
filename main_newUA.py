...
def loadDataFromFiles():
    ...
    header = '-----------------------------------------------------------------------------------------\n'
    header += 'Код ринку       Назва ринку          Дата              Картопля     Капуста     Цибуля'
    header += '\n-----------------------------------------------------------------------------------------\n'
...
def sortTable():
    global costs_data
    sort_mode = input('0 - сортування за ціною картоплі, 1 - капусти, 2 - цибулі, 3 - номер ринка:\n')
    if sort_mode == '0':
...
def waitForCommand():
    print('\n0 - відкрити таблицю, 1 - добавити нові данні, 2 - сортування таблиці, 3 - зберегти данні\n4 - вихід')
    command_number = input('Введіть номер команди: ')
    if command_number == '0':
        programStartScreen()

    elif command_number == '1':
        market_id = input('Введите код ринка: ')
        if market_id not in markets_data.keys():
            command = input('Ринок з таким кодоє вже є в базі данних. 1 - додати новий, або - відмінити: ')
            if command is '1':
                markets_data[market_id] = input('Введіть назу для нового ринка: ')
            else:
                waitForCommand()

        addNewCosts(market_id, input('Ціна на картоплю: '), input('Ціна на капусту: '),
                    input('Цена на цибулю: '))
        programStartScreen()
